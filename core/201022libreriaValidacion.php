 ...
 */
    ...
    // Confirma el password introducido anteriormente
    // Si $passwd no era válido o si no coincide con $confirmarPasswd devuelve error
    /**
     * @function confirmarPassword();
     * @author Nacho del Prado Losada
     * @param $confirmarPasswd password a comprobar.
     * @param $passwd password a confirmar.
     * @param $obligatorio valor booleano indicado mediante 1, si es obligatorio o 0 si no lo es.
     * @return null|string Devuelve null si es correcto o un mensaje de error en caso de que lo haya.
     */
    public static function confirmarPassword($confirmarPasswd, $passwd, $obligatorio = 0) {  
        $mensajeError = null;
        if ($obligatorio == 1) {
            $mensajeError = self::comprobarNoVacio($passwd); 
        }
        if ($confirmarPasswd != $passwd){
            $mensajeError = "Las contraseñas no coinciden.";
        }
        if(self::validarPassword($passwd)){
